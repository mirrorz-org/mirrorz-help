...
```bash
echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/ros-archive-keyring.gpg] {{http_protocol}}{{mirror}}/ubuntu {{release_name}} main" | {{sudo}}tee /etc/apt/sources.list.d/ros2.list > /dev/null
