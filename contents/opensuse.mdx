...
openSUSE 默认启用了 [RIS](https://en.opensuse.org/openSUSE:Standards_Repository_Index_Service) 和 MirrorCache 技术用于统一分发软件仓库更改和镜像，**无需更改镜像**。以下所有操作在下次镜像更新时将被覆盖或重置。

本方法中提供了一种禁用 RIS 的方法来实现临时禁用 RIS 自动刷新。
...
如许还原为传统镜像源管理方式，确保在 `bash` 或 `zsh` 下执行：
...
5. `软件源名称` （Repository Name） 和 `URL` 填写为 [Leap](#openSUSE-Leap-使用方法) 或 [Tumbleweed](#opensuse-tumbleweed-使用方法) 中的仓库和对应连接，如：
    ...
    |发行版|仓库名称|URL|备注|
    |:-:|:-:|:-:|:-:|
    |Leap|OSS|{{http_protocol}}{{mirror}}/distribution/leap/$releasever/repo/oss/|-|
    |Leap|NON-OSS|{{http_protocol}}{{mirror}}/distribution/leap/$releasever/repo/non-oss/|-|
    |Leap|UPDATE|{{http_protocol}}{{mirror}}/update/leap/$releasever/oss/|-|
    |Leap|UPDATE-NON-OSS|{{http_protocol}}{{mirror}}/update/leap/$releasever/non-oss/|-|
    |Leap|SLE-UPDATE|{{http_protocol}}{{mirror}}/update/leap/$releasever/sle/|仅使用 Backport 时需要添加|
    |Leap|SLE-BACKPORT-UPDATE|{{http_protocol}}{{mirror}}/update/leap/$releasever/backports/|仅使用 Backport 时需要添加|
    |Tumbleweed|OSS|{{http_protocol}}{{mirror}}/tumbleweed/repo/oss/|-|
    |Tumbleweed|NON-OSS|{{http_protocol}}{{mirror}}/tumbleweed/repo/non-oss/|-|
    |Tumbleweed|UPDATE|{{http_protocol}}{{mirror}}/update/tumbleweed/|-|
    |Slowroll|OSS|{{http_protocol}}{{mirror}}/slowroll/repo/oss/|-|
    |Slowroll|NON-OSS|{{http_protocol}}{{mirror}}/slowroll/repo/non-oss/|-|
    |Slowroll|UPDATE|{{http_protocol}}{{mirror}}/update/slowroll/repo/oss/|-|
