...
由于 RIS 的存在，手工关闭单个官方源仓库的刷新，将在下次执行更新操作时被重新启用。仍需要和官方源（download.opensuse.org）进行同步，因此在不更改 `openSUSE` 服务的前提下，所有针对官方源的修改将在下次仓库或软件更新时被覆盖或重置。

下面流程提供了一种半永久性禁用 RIS 的方法,但同时保留了 `zypper` 的 `RIS` 特性满足官方的推荐更新方式，来实现临时禁用 RIS 自动刷新。
